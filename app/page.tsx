...
export default function Home() {
  ...
  const page = useMemo(() => {
    ...
    if (screen === "vehicle" && vehicle) return <VehicleOverview vehicle={vehicle} services={services} reminders={data.reminders.filter((item) => item.vehicleId === vehicle.id)} parts={data.parts.filter((item) => item.vehicleId === vehicle.id)} fuel={fuelEntries} onAdd={() => setModal("service")} onEdit={() => setModal("editVehicle")} onClearCloud={clearSelectedVehicleCloudData} cloudEnabled={account.signedIn} />;
...
function VehicleOverview({ vehicle, services, reminders, parts, fuel, onAdd, onEdit, onClearCloud, cloudEnabled }: { vehicle: Vehicle; services: Service[]; reminders: Reminder[]; parts: Part[]; fuel: FuelEntry[]; onAdd: () => void; onEdit: () => void; onClearCloud: () => void; cloudEnabled: boolean }) { return <div className="stack"><div className="heroCard"><VehicleArt vehicle={vehicle} /><div className="heroInfo"><h2>{vehicle.nickname}</h2><p>{vehicle.year} {vehicle.make} {vehicle.model} {vehicle.trim}</p></div></div><div className="profileGrid"><Info label="VIN" value={vehicle.vin || "Not added"} /><Info label="Plate" value={vehicle.plate || "Not added"} /><Info label="Mileage" value={`${vehicle.mileage.toLocaleString()} mi`} /><Info label="Notes" value={vehicle.notes || "No notes yet"} /></div><div className="quickActions fixed"><button onClick={onAdd}>Log service</button><button onClick={onEdit}>Edit details</button><button className="dangerInline" onClick={onClearCloud} disabled={!cloudEnabled}>Clear cloud</button></div><Section title="Reminders">{reminders.length ? reminders.map((reminder) => <ReminderCard key={reminder.id} reminder={reminder} />) : <EmptyState title="No reminders" text="Add time or mileage reminders for upcoming service." />}</Section><Section title="Fuel">{fuel.length ? <FuelSummary entries={fuel} /> : <EmptyState title="No fuel logged" text="Track fill-ups to calculate MPG and fuel cost per mile." />}</Section><Section title="Purchased parts">{parts.length ? parts.map((part) => <PartCard key={part.id} part={part} vehicle={vehicle} />) : <EmptyState title="No parts saved" text="Purchased parts will appear here when linked to this vehicle." />}</Section><Section title="History">{services.length ? services.map((service) => <ServiceCard key={service.id} service={service} vehicle={vehicle} />) : <EmptyState title="No history yet" text="Your service records will build the ownership story for this vehicle." />}</Section></div>; }
