...
import { deleteAllCloudData, deleteCloudTableData, deleteCloudVehicleData, hardDeleteRecord, listRecords, upsertRecord } from "../lib/supabase/storage";
...
type CloudClearTarget = "all" | "vehicle" | "vehicles" | "services" | "reminders" | "parts" | "projects" | "wishlist_items" | "fuel_entries";
...
function isMissingFuelTable(error: unknown) {
  if (!error || typeof error !== "object") return false;
  const message = "message" in error ? String((error as { message?: unknown }).message ?? "") : "";
  const code = "code" in error ? String((error as { code?: unknown }).code ?? "") : "";
  return code === "42P01" || /fuel_entries|schema cache|could not find the table/i.test(message);
}
function friendlySyncError(error: unknown) {
  if (!error || typeof error !== "object") return "Sync failed";
...
export default function Home() {
  ...
  useEffect(() => {
    if (!hasLoaded || !account.signedIn) return;
    if (autoSyncTimer.current) clearTimeout(autoSyncTimer.current);
    autoSyncTimer.current = setTimeout(() => { syncDataToSupabase(data, false); }, 900);
    return () => { if (autoSyncTimer.current) clearTimeout(autoSyncTimer.current); };
  }, [data, hasLoaded, account.signedIn]);
  ...
  async function clearCloudData(targetType: CloudClearTarget, targetVehicleId: string) {
    if (!account.signedIn) return setToast("Sign in before clearing cloud data");
    const labels: Record<Exclude<CloudClearTarget, "vehicle">, string> = { all: "all CarKeep cloud data", vehicles: "all vehicle profiles", services: "all service history", reminders: "all reminders", parts: "all purchased parts", projects: "all projects", wishlist_items: "all wishlist items", fuel_entries: "all fuel entries" };
    let label = targetType === "vehicle" ? "" : labels[targetType];
    let action: Promise<{ error: unknown }>;
    if (targetType === "vehicle") {
      const target = data.vehicles.find((item) => item.id === targetVehicleId);
      if (!target) return setToast("Choose a vehicle first");
      label = `${target.nickname} and all linked cloud records`;
      action = deleteCloudVehicleData(target.id);
    } else if (targetType === "all") {
      action = deleteAllCloudData();
    } else {
      action = deleteCloudTableData(targetType);
    }
    const ok = confirm(`Clear ${label} from Supabase? Local data on this device will stay.`);
    if (!ok) return;
    const { error } = await action;
    setToast(error ? friendlySyncError(error) : `Cleared ${label}`);
  }
  async function syncDataToSupabase(nextData: AppData, announce: boolean) {
    if (!supabase || !account.signedIn) { if (announce) setToast("Sign in before syncing"); return false; }
    const jobs = [
      ...nextData.vehicles.map((item) => upsertRecord("vehicles", item)),
      ...nextData.services.map((item) => upsertRecord("services", item)),
      ...nextData.reminders.map((item) => upsertRecord("reminders", item)),
      ...nextData.parts.map((item) => upsertRecord("parts", item)),
      ...nextData.projects.map((item) => upsertRecord("projects", item)),
      ...nextData.wishlist.map((item) => upsertRecord("wishlist_items", item)),
      ...nextData.fuel.map((item) => upsertRecord("fuel_entries", item)),
    ];
    const results = await Promise.all(jobs);
    const failed = results.find((result) => result.error && !isMissingFuelTable(result.error));
    const missingFuel = results.some((result) => result.error && isMissingFuelTable(result.error));
    if (failed) { if (announce) setToast(friendlySyncError(failed.error)); return false; }
    if (announce) setToast(missingFuel ? "Synced. Fuel needs the Supabase schema update." : "Synced to Supabase");
    return true;
  }
  async function syncToSupabase() { await syncDataToSupabase(data, true); }
  async function restoreFromSupabase() {
    if (!supabase || !account.signedIn) return setToast("Sign in before restoring");
    const [vehiclesCloud, servicesCloud, remindersCloud, partsCloud, projectsCloud, wishlistCloud, fuelCloud] = await Promise.all([
      listRecords<Vehicle>("vehicles"),
      listRecords<Service>("services"),
      listRecords<Reminder>("reminders"),
      listRecords<Part>("parts"),
      listRecords<Project>("projects"),
      listRecords<WishlistItem>("wishlist_items"),
      listRecords<FuelEntry>("fuel_entries"),
    ]);
    const failed = [vehiclesCloud, servicesCloud, remindersCloud, partsCloud, projectsCloud, wishlistCloud, fuelCloud].find((result) => result.error);
    if (failed?.error) return setToast(friendlySyncError(failed.error));
    const cloudData: AppData = { vehicles: vehiclesCloud.data, services: servicesCloud.data, reminders: remindersCloud.data, parts: partsCloud.data, projects: projectsCloud.data, wishlist: wishlistCloud.data, fuel: fuelCloud.data };
    const count = cloudData.vehicles.length + cloudData.services.length + cloudData.reminders.length + cloudData.parts.length + cloudData.projects.length + cloudData.wishlist.length + cloudData.fuel.length;
    if (!count) return setToast("No cloud data found");
    setData((current) => {
      const next = mergeBackup(current, cloudData);
      setVehicleId(next.vehicles[0]?.id ?? "");
      return next;
    });
    setToast("Cloud garage restored");
  }
  ...
  const page = useMemo(() => {
    if (!hasVehicles && !["dashboard", "garage", "more", "settings"].includes(screen)) return <StartScreen onAddVehicle={() => setModal("vehicle")} onOpenSettings={() => setScreen("settings")} />;
    ...
    if (screen === "wishlist") return <Wishlist data={data} setData={setData} onAdd={() => setModal("wishlist")} onCloudDelete={(id) => { if (account.signedIn) hardDeleteRecord("wishlist_items", id); }} />;
    if (screen === "reminders") return <Reminders data={data} onAdd={() => setModal("reminder")} />;
    if (screen === "settings") return <Settings data={data} account={account} onCreateAccount={createAccount} onPasswordSignIn={signInWithPassword} onSignOut={signOut} onSync={syncToSupabase} onRestore={restoreFromSupabase} onBackup={exportBackup} onImport={importBackup} onClear={() => { repository.clear(); setData(emptyData); setVehicleId(""); setToast("Local data cleared"); }} onClearCloud={clearCloudData} />;
...
function StartScreen({ onAddVehicle, onOpenSettings }: { onAddVehicle: () => void; onOpenSettings: () => void }) { return <div className="stack"><div className="emptyHero"><p className="eyebrow">Start clean</p><h2>Add your first vehicle</h2><p>CarKeep starts empty, but you can sign in first to sync or restore your garage from Supabase.</p><div className="emptyActions"><button className="primary" onClick={onAddVehicle}>Add vehicle</button><button onClick={onOpenSettings}>Sign in or restore</button></div></div><div className="setupList"><span>1. Sign in or add a vehicle</span><span>2. Sync your garage anywhere</span><span>3. Track service, fuel, parts, and projects</span></div></div>; }
...
function fuelPeriods(entries: FuelEntry[]) {
  const sorted = [...entries].sort((a, b) => a.odometer - b.odometer || a.date.localeCompare(b.date));
  let anchor: FuelEntry | null = null;
  const periods = new Map<string, { miles: number; gallons: number; mpg: number }>();
  for (const entry of sorted) {
    if (!entry.fullTank) continue;
    if (anchor) {
      const miles = entry.odometer - anchor.odometer;
      if (miles > 0 && entry.gallons > 0) periods.set(entry.id, { miles, gallons: entry.gallons, mpg: miles / entry.gallons });
    }
    anchor = entry;
  }
  return periods;
}
function fuelStats(entries: FuelEntry[]) {
  const sorted = [...entries].sort((a, b) => a.odometer - b.odometer || a.date.localeCompare(b.date));
  const periods = Array.from(fuelPeriods(sorted).values());
  const spend = sorted.reduce((sum, item) => sum + item.totalCost, 0);
  const gallons = sorted.reduce((sum, item) => sum + item.gallons, 0);
  const measuredMiles = periods.reduce((sum, item) => sum + item.miles, 0);
  const measuredGallons = periods.reduce((sum, item) => sum + item.gallons, 0);
  const totalDistance = sorted.length > 1 ? Math.max(0, sorted[sorted.length - 1].odometer - sorted[0].odometer) : 0;
  return { spend, gallons, measuredMiles, avgMpg: measuredGallons > 0 ? measuredMiles / measuredGallons : 0, costPerMile: totalDistance > 0 ? spend / totalDistance : 0 };
}
...
function Fuel({ data, vehicle, onAdd }: { data: AppData; vehicle?: Vehicle; onAdd: () => void }) { const entries = vehicle ? data.fuel.filter((item) => item.vehicleId === vehicle.id).sort((a, b) => b.odometer - a.odometer || b.date.localeCompare(a.date)) : data.fuel.sort((a, b) => b.odometer - a.odometer || b.date.localeCompare(a.date)); const stats = fuelStats(entries); const periods = fuelPeriods(entries); return <div className="stack"><button className="primary" onClick={onAdd}>Log fuel</button><div className="bigNumber"><span>Fuel spend</span><strong>{money(stats.spend)}</strong><p>{stats.avgMpg ? `${stats.avgMpg.toFixed(1)} MPG average from full-tank fill-ups · $${stats.costPerMile.toFixed(2)} per mile` : "Log two full-tank fill-ups to calculate MPG."}</p></div><FuelSummary entries={entries} /><Section title="Fill-up history">{entries.length ? entries.map((entry) => { const period = periods.get(entry.id); return <article className="card fuelCard" key={entry.id}><div><span className="badge">{entry.fullTank ? "Full tank" : "Partial"}</span><h2>{period ? `${period.mpg.toFixed(1)} MPG` : `${entry.gallons.toFixed(2)} gal`}</h2><p>{entry.date} · {entry.odometer.toLocaleString()} mi · {entry.gallons.toFixed(2)} gal</p><small>{period ? `${period.miles.toLocaleString()} mi since last full tank` : entry.fullTank ? "Needs previous full tank for MPG" : "Partial fill not used for MPG"}{entry.notes ? ` · ${entry.notes}` : ""}</small></div><strong>{money(entry.totalCost)}</strong></article>; }) : <EmptyState title="No fuel entries" text="Log gallons, price, odometer, and notes for each fill-up." action="Log fuel" onAction={onAdd} />}</Section></div>; }
...
function Wishlist({ data, setData, onAdd, onCloudDelete }: { data: AppData; setData: React.Dispatch<React.SetStateAction<AppData>>; onAdd: () => void; onCloudDelete: (id: string) => void }) { return <div className="stack"><button className="primary" onClick={onAdd}>Add wishlist item</button>{data.wishlist.length ? data.wishlist.map((item) => { const best = Math.min(...item.retailers.map((retailer) => retailer.price)); return <article className="card" key={item.id}><span className={`badge ${item.priority.toLowerCase()}`}>{item.priority}</span><h2>{item.name}</h2><p>{data.vehicles.find((vehicle) => vehicle.id === item.vehicleId)?.nickname} · est. {money(item.estimate)}</p><div className="retailers">{item.retailers.map((retailer) => <span key={retailer.name}>{retailer.name} <b>{money(retailer.price)}</b>{retailer.price === best && <em>Best</em>}</span>)}</div><button onClick={() => { onCloudDelete(item.id); setData((current) => ({ ...current, parts: [{ id: uid(), vehicleId: item.vehicleId, name: item.name, category: "Wishlist purchase", price: best, purchaseDate: new Date().toISOString().slice(0, 10), retailer: item.retailers.find((retailer) => retailer.price === best)?.name ?? "Retailer", order: "", warranty: "", installed: false, link: "", notes: item.notes }, ...current.parts], wishlist: current.wishlist.filter((wishlistItem) => wishlistItem.id !== item.id) })); }}>Move to purchased parts</button></article>; }) : <EmptyState title="No wishlist items" text="Compare future parts and prices before you buy." />}</div>; }
...
function Settings({ data, account, onCreateAccount, onPasswordSignIn, onSignOut, onSync, onRestore, onBackup, onImport, onClear, onClearCloud }: { data: AppData; account: AccountState; onCreateAccount: (email: string, password: string) => void; onPasswordSignIn: (email: string, password: string) => void; onSignOut: () => void; onSync: () => void; onRestore: () => void; onBackup: () => void; onImport: (file: File, mode: "replace" | "merge") => void; onClear: () => void; onClearCloud: (targetType: CloudClearTarget, vehicleId: string) => void }) {
  const [email, setEmail] = useState(account.email);
  const [password, setPassword] = useState("");
  const [cloudVehicleId, setCloudVehicleId] = useState(data.vehicles[0]?.id ?? "");
  const [cloudClearTarget, setCloudClearTarget] = useState<CloudClearTarget>("vehicle");
  ...
  return <div className="stack settingsStack"><div className="card"><h2>Account</h2><p>{isSupabaseConfigured ? "Sign in with email and password to sync your garage across devices." : "Supabase is not configured yet. Add the env vars in Vercel to enable login."}</p><div className="accountBox">{account.signedIn ? <><strong>{account.email}</strong><button onClick={onSignOut}>Sign out</button></> : <><input value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" type="email" autoComplete="email" /><input value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Password" type="password" autoComplete="current-password" minLength={6} /><div className="authActions"><button onClick={() => onPasswordSignIn(email, password)}>Sign in</button><button onClick={() => onCreateAccount(email, password)}>Create account</button></div></>}{account.message && <small>{account.message}</small>}</div></div><div className="card dataCard"><div><h2>Data</h2><p>Local-first storage with JSON backup and optional Supabase sync.</p></div><div className="grid2"><Stat label="Records" value={String(recordCount)} /><Stat label="Storage" value={account.signedIn ? "Local + cloud" : "Local"} /></div></div><div className="settingsActions"><button className="primary" onClick={onSync}>Sync now</button><button onClick={onRestore}>Restore cloud</button><button onClick={onBackup}>Export backup</button><button onClick={() => chooseBackup("merge")}>Import merge</button><button onClick={() => { if (confirm("Replace everything in CarKeep with this backup?")) chooseBackup("replace"); }}>Import replace</button></div><input ref={fileInput} className="hiddenFile" type="file" accept="application/json,.json" onChange={(event) => { const file = event.currentTarget.files?.[0]; const mode = event.currentTarget.dataset.mode === "replace" ? "replace" : "merge"; if (file) onImport(file, mode); }} /><div className="dangerPanel"><button className="danger" onClick={onClear}>Delete local data</button><div className="card dangerZone"><h2>Cloud data</h2><p>Clear one car, one category, or everything from Supabase. The copy on this device stays here.</p><label>Clear target<select value={cloudClearTarget} onChange={(event) => setCloudClearTarget(event.target.value as CloudClearTarget)}><option value="vehicle">One vehicle and linked records</option><option value="all">Everything in cloud</option><option value="vehicles">Vehicle profiles only</option><option value="services">Service history</option><option value="reminders">Reminders</option><option value="parts">Purchased parts</option><option value="projects">Projects</option><option value="wishlist_items">Wishlist</option><option value="fuel_entries">Fuel entries</option></select></label>{cloudClearTarget === "vehicle" && <label>Vehicle<select value={cloudVehicleId} onChange={(event) => setCloudVehicleId(event.target.value)}>{data.vehicles.length ? data.vehicles.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.nickname} · {vehicle.year} {vehicle.make} {vehicle.model}</option>) : <option value="">No vehicles</option>}</select></label>}<button className="danger" disabled={!account.signedIn || (cloudClearTarget === "vehicle" && !cloudVehicleId)} onClick={() => onClearCloud(cloudClearTarget, cloudVehicleId)}>Clear cloud data</button></div></div></div>;
}
