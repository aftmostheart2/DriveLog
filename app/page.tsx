...
type ModalType = "vehicle" | "editVehicle" | "service" | "part" | "project" | "reminder" | "wishlist" | "backup";
...
export default function Home() {
  ...
  async function clearSelectedVehicleCloudData() {
    if (!vehicle) return setToast("Choose a vehicle first");
    if (!account.signedIn) return setToast("Sign in before clearing cloud data");
    const ok = confirm(`Clear cloud data for ${vehicle.nickname}? Local data on this device will stay.`);
    if (!ok) return;
    const { error } = await deleteCloudVehicleData(vehicle.id);
  ...
  const page = useMemo(() => {
    ...
    if (screen === "vehicle" && vehicle) return <VehicleOverview vehicle={vehicle} services={services} reminders={data.reminders.filter((item) => item.vehicleId === vehicle.id)} parts={data.parts.filter((item) => item.vehicleId === vehicle.id)} onAdd={() => setModal("service")} onEdit={() => setModal("editVehicle")} onClearCloud={clearSelectedVehicleCloudData} cloudEnabled={account.signedIn} />;
    ...
    if (screen === "settings") return <Settings data={data} account={account} onCreateAccount={createAccount} onPasswordSignIn={signInWithPassword} onSignOut={signOut} onSync={syncToSupabase} onBackup={exportBackup} onImport={importBackup} onClear={() => { repository.clear(); setData(emptyData); setVehicleId(""); setToast("Local data cleared"); }} />;
...
function VehicleOverview({ vehicle, services, reminders, parts, onAdd, onEdit, onClearCloud, cloudEnabled }: { vehicle: Vehicle; services: Service[]; reminders: Reminder[]; parts: Part[]; onAdd: () => void; onEdit: () => void; onClearCloud: () => void; cloudEnabled: boolean }) { return <div className="stack"><div className="heroCard"><VehicleArt vehicle={vehicle} /><div className="heroInfo"><h2>{vehicle.nickname}</h2><p>{vehicle.year} {vehicle.make} {vehicle.model} {vehicle.trim}</p></div></div><div className="profileGrid"><Info label="VIN" value={vehicle.vin || "Not added"} /><Info label="Plate" value={vehicle.plate || "Not added"} /><Info label="Mileage" value={`${vehicle.mileage.toLocaleString()} mi`} /><Info label="Notes" value={vehicle.notes || "No notes yet"} /></div><div className="quickActions fixed"><button onClick={onAdd}>Log service</button><button onClick={onEdit}>Edit details</button><button className="dangerInline" onClick={onClearCloud} disabled={!cloudEnabled}>Clear cloud</button></div><Section title="Reminders">{reminders.length ? reminders.map((reminder) => <ReminderCard key={reminder.id} reminder={reminder} />) : <EmptyState title="No reminders" text="Add time or mileage reminders for upcoming service." />}</Section><Section title="Purchased parts">{parts.length ? parts.map((part) => <PartCard key={part.id} part={part} vehicle={vehicle} />) : <EmptyState title="No parts saved" text="Purchased parts will appear here when linked to this vehicle." />}</Section><Section title="History">{services.length ? services.map((service) => <ServiceCard key={service.id} service={service} vehicle={vehicle} />) : <EmptyState title="No history yet" text="Your service records will build the ownership story for this vehicle." />}</Section></div>; }
...
function Settings({ data, account, onCreateAccount, onPasswordSignIn, onSignOut, onSync, onBackup, onImport, onClear }: { data: AppData; account: AccountState; onCreateAccount: (email: string, password: string) => void; onPasswordSignIn: (email: string, password: string) => void; onSignOut: () => void; onSync: () => void; onBackup: () => void; onImport: (file: File, mode: "replace" | "merge") => void; onClear: () => void }) {
